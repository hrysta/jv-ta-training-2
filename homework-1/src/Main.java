...
public class Main {
    ...
    public static int[] strToInts(String inputString) {
    ...
    public static void rangeSearch(int[] array, int size) {

        int counter = 0;
        for (int i = 1; i <= size - 1; i++) {

            int monotone = Math.abs(array[i] - array[i-1]);

            if (monotone != 1 || i == (size - 1)) {
                if (counter == 0) {
